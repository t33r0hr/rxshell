...
class ProcessWrapper {
    ...
    get stream() {
        return rxjs_1.Observable.merge(this.__onStdoutData.map(event => {
    ...
    bindChildProcess() {
        this.__onFail = rxjs_1.Observable.fromEvent(this.childProcess, 'error', (error) => ({ error }));
        this.__onClose = rxjs_1.Observable.fromEvent(this.childProcess, 'close', (code, signal) => ({ code, signal }));
        this.__onStdoutData = fromReadable_1.fromReadable(this.childProcess.stdout).map(data => ({ data }));
        this.__onStderrData = fromReadable_1.fromReadable(this.childProcess.stderr).map(data => ({ data }));
